...
docker build \
  --network=host \
  -t "$NEXUS_DOCKER_REGISTRY/auth-proxy:$BUILD_VERSION" -f $DOCKER_FILE \
  . \
  && \
  
docker tag \
  "$NEXUS_DOCKER_REGISTRY/auth-proxy:$BUILD_VERSION" "$NEXUS_DOCKER_REGISTRY/auth-proxy:latest" \
  
&& docker push \
  "$NEXUS_DOCKER_REGISTRY/auth-proxy" \
